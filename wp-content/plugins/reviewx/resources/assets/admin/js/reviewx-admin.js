(function ($) {
	...
	var RX_Admin = {
		...
		tabChanger: function (button) {
			...
			if (currentPage == 'reviewx-review-email') {
				if (tabKey == 'scheduled_emails') {
					...
					$('.rx-form-builder-section').css({
						'flex': '0 0 100%'
					});
					$('.rx_review_email_content_wrap .rx_review_email_content').css({
						'background-color': '#fff'
					});
					$('.rx_review_email_content_wrap').css({
						'padding': '0'
					});
					$('.rx_review_email_content .rx-meta-section table tr td').css({
						'padding': '0'
					});
					$('.rx_review_email_content .rx-meta-section table table tr td').css({
						'padding': '5px 30px'
					});
					$('#rx-option-disable_autocreate_unsubscribe_page .rx-control').css({
						'padding-left': '0'
					});
		...
		resetSection: function (button) {
			var button = $(button),
				...
				fields = parent.find('.rx-meta-field'),
				updateFields = [];
	...
	$('body').on('click',
		'.is-pro td > div > label, .rx-pro-checkbox table tr td, .rx-radio-pro, #rx-admin-upload-photo, #rx-admin-upload-video, .rx_admin_remove_image, .rx_admin_remove_video, .rviwx-rating-column__item-field, .rx_highlight_switch, .rx-media-upload-button, .reviewx_recommended_list .reviewx_radio',
		function (e) {
			e.preventDefault();
