...
/**
 * To change add to cart text & icon on product archives(Collection) page
 */
...
{
    global $product;
    // $link = $product->get_permalink(); //For details page
    $link = $product->add_to_cart_url();
    echo do_shortcode('<a href="' . $link . '" class="button product_type_simple add_to_cart_button ajax_add_to_cart"><i class="bi bi-bag-check"></i> Add To Cart</a>');
}
...
{ ?>
    <div class="quantity-area">
        <h6><?php echo esc_html__('Quantity', 'triprex'); ?></h6>
    </div>
<?php
}
...
{ ?>
    <?php if ((Egns\Helper\Egns_Helper::egns_get_theme_option('payment_method_widget_enable') == 1) && class_exists('CSF')) : ?>

    <?php
        // Get the gallery field value using the Egns_Helper class
        $image_gallery = Egns\Helper\Egns_Helper::egns_get_theme_option('image_gallery');
        $gallery_ids   = explode(',', $image_gallery);

        // Check if the field has any images
        if (!empty($image_gallery)) {
            echo '<div class="payment-method">';
            echo '<h6>' . wp_kses(Egns\Helper\Egns_Helper::egns_get_theme_option('payment_method_heading'), wp_kses_allowed_html('post')) . '</h6>';
    ...
    <ul class="product-shipping-delivers">
        <li class="product-shipping">
            ...
            <?php echo wp_kses(Egns\Helper\Egns_Helper::egns_get_theme_option('shipping_text'), wp_kses_allowed_html('post')) ?>
        </li>
        <li class="product-delivers">
            ...
            <p>
                <?php echo wp_kses(Egns\Helper\Egns_Helper::egns_get_theme_option('delivers_text'), wp_kses_allowed_html('post')) ?>
                <a href="<?php echo wp_kses(Egns\Helper\Egns_Helper::egns_get_theme_option('delivers_btn_url'), wp_kses_allowed_html('post')) ?>"><?php echo wp_kses(Egns\Helper\Egns_Helper::egns_get_theme_option('delivers_btn_text'), wp_kses_allowed_html('post')) ?></a>
            </p>
        </li>
    </ul>
    <?php
...
function add_custom_addtocart_and_checkout()
{
    ...
    $button_text   = __("Buy now", "triprex");
 ...
 */
function triprex_woocommerce_breadcrumbs()
{
    return array(
        'delimiter'   => ' &#10170; ',
        ...
        'before'      => '',
        'after'       => '',
        'home'        => _x('Home', 'breadcrumb', 'triprex'),
    );
}
add_filter('woocommerce_breadcrumb_defaults', 'triprex_woocommerce_breadcrumbs');


/**
 * WooCommerce product archive page title link add
 */
function triprexChangeProductsTitle()
{
    ?>
...
add_action('woocommerce_shop_loop_item_title', 'triprexChangeProductsTitle', 10);
...
function triprex_display_quantity_plus()
{
    echo '<button type="button" class="plus"><i class="bi bi-plus-lg"></i></button>';
}
add_action('woocommerce_after_quantity_input_field', 'triprex_display_quantity_plus');

function triprex_display_quantity_minus()
{
    echo '<button type="button" class="minus"><i class="bi bi-dash-lg"></i></button>';
}
add_action('woocommerce_before_quantity_input_field', 'triprex_display_quantity_minus');
...
function display_percentage_on_sale_badge($html, $post, $product)
{
    ...
    return '<span class="onsale">' . esc_html__('-', 'triprex') . ' ' . $percentage . '</span>'; // If needed then change or remove "up to -" text
}
...
if (class_exists('WooCommerce', false)) {
    ...
    function triprex_add_cart_quantity_plus_minus()
    {
    ...
    add_action('wp_footer', 'triprex_add_cart_quantity_plus_minus');
}
