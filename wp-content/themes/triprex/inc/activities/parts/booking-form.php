<?php
$get_pricing_categories = Egns\Helper\Egns_Helper::egns_activities_value('activities_booking_re');
$get_activities_extra_services = Egns\Helper\Egns_Helper::egns_activities_value('activities_extra_service_re');
$query_var = get_transient('triprex_activities_query_var');
$activities_date = null;
if( !empty( $query_var['activities_date'] ) ) {
    $activities_date = $query_var['activities_date'];
}
delete_transient( 'triprex_activities_query_var' );
    ...
    <form>
        ...
        <div class="tour-date-wrap mb-50">
            ...
            <div class="form-inner mb-20">
                <div class="form-group">
                    <input type="hidden" name="activities_date" value="<?php echo sprintf('%s', $activities_date) ?>">
                    <input type="text" name="inOut" placeholder="<?php echo \Egns\Helper\Egns_Helper::get_gmt_formatted_date() ?>">
        ...
        <div class="booking-form-item-type mb-45">
            <?php if (!empty($get_pricing_categories) && count($get_pricing_categories) > 0) : ?>
                <?php foreach ($get_pricing_categories as $category) : ?>
                    <?php if (!empty($category['activities_pricing_category'])) : ?>
                        <div class="number-input-item adults">
                            <label class="number-input-lable">
                                <?php echo Egns\Helper\Egns_Helper::egns_get_taxonomy_data($category['activities_pricing_category'], true); ?>
                                <?php if (isset($category['activities_service_t_price_sale_check']) && $category['activities_service_t_price_sale_check'] == 1) : ?>
                                    <?php echo \Egns\Helper\Egns_Helper::egns_get_package_price($category['activities_price'], $category['activities_service__t_price_sale']); ?>
                                <?php else : ?>
                                    <?php echo \Egns\Helper\Egns_Helper::egns_get_package_price($category['activities_price']); ?>
                                <?php endif ?>
                            </label>
                            <div class="quantity-counter">
                                <a href="#" class="quantity__minus"><i class="bi bi-dash"></i></a>
                                <input name="quantity" min="<?php echo !empty($category['activities_minimum_quantity']) ? $category['activities_minimum_quantity'] : 0 ?>" pricing-category-id="<?php echo esc_attr($category['activities_pricing_category']); ?>" type="text" class="quantity__input" value="<?php echo !empty($category['activities_minimum_quantity']) ? str_pad($category['activities_minimum_quantity'], 2, '0', STR_PAD_LEFT) : '00'; ?>">
        ...
        <div class="booking-form-item-type">
            ...
            <div class="checkbox-container">
                <?php foreach ($get_activities_extra_services as $key => $services) : ?>
                    <?php if (!empty($services['activities_service_price'])) : ?>
                        <label class="check-container">
                            <?php if (!empty($services['activities_extra_service'])) : ?>
                                <?php echo sprintf('%s', $services['activities_extra_service']) ?>
                            <?php endif ?>
                            <input type="checkbox" class="services_check" name="services_list" service-slug="<?php echo \Egns\Helper\Egns_Helper::egns_slugify($services['activities_extra_service']) ?>" value="<?php echo sprintf('%d', $key) ?>">
                            <span class="checkmark"></span>
                            <span class="price"><?php echo \Egns\Helper\Egns_Helper::egns_get_package_price($services['activities_service_price']); ?></span>
                        </label>
                    <?php endif ?>
                <?php endforeach ?>
