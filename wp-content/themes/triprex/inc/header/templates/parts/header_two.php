...
<header class="header-area style-2 <?php echo sprintf('%s',$breadcrumb_enable_by_page == 1)  ? '' : 'disable' ?>">
    <div class="header-logo">
        <?php
        if (!empty(Egns_Helper::egns_page_option_value('page_header_two_logo', 'url'))) {
            Egns_Helper::get_theme_logo(Egns_Helper::egns_page_option_value('page_header_two_logo', 'url'));
        } else {
            if (!empty(Egns_Helper::egns_get_theme_option('header_logo', 'url'))) {
                Egns_Helper::get_theme_logo(Egns_Helper::egns_get_theme_option('header_logo', 'url'));
            } else {
                Egns_Helper::get_theme_logo(NULL);
            }
        }
        ?>
    </div>
    <div class="main-menu">
        <div class="mobile-logo-area d-lg-none d-flex justify-content-between align-items-center">
            <div class="mobile-logo-wrap">
                <?php
                if (!empty(Egns_Helper::egns_page_option_value('page_header_two_logo_mobile', 'url'))) {
                    Egns_Helper::get_theme_logo(Egns_Helper::egns_page_option_value('page_header_two_logo_mobile', 'url'));
                } else {
                    if (!empty(Egns_Helper::egns_get_theme_option('header_logo_mobile', 'url'))) {
                        Egns_Helper::get_theme_logo(Egns_Helper::egns_get_theme_option('header_logo_mobile', 'url'));
                    } else {
                        Egns_Helper::get_theme_logo(NULL);
                    }
                }
        ...
        <!-- Main Menu -->
        <?php Egns_Helper::egns_get_theme_menu('primary-menu', '', '<i class="bi bi-plus dropdown-icon"></i>', 'menu-list', 3); ?>
        <?php if (class_exists('CSF') && class_exists('woocommerce') && Egns_Helper::egns_get_theme_option('header_signin_show') == 1) : ?>
            <div class="topbar-right d-lg-none d-block">
                <?php Egns_Helper::mobile_login_register() ?>
            </div>
        <?php endif ?>

        <?php if (class_exists('CSF') && (Egns_Helper::egns_get_theme_option('header_contact_info_show') == 1)) :  ?>
            <?php foreach ((array) Egns_Helper::egns_get_theme_option('header_contact_info') as $item) : ?>
                <?php
                    ...
                    <?php elseif ($contactType === 'others') : ?>
                        <div class="hotline-area d-lg-none d-flex">
                            ...
                            <div class="content">
                                ...
                                <?php endif ?>
                                <h6><a href="<?php echo esc_html($item['custom_info']); ?>"><?php echo esc_html($item['custom_info']); ?></a></h6>
                            </div>
                        </div>
                    <?php endif ?>
                <?php endif ?>
    ...
    <div class="nav-right d-flex jsutify-content-end align-items-center">
        <ul class="icon-list">
            <?php if (class_exists('CSF') && class_exists('woocommerce') && Egns_Helper::egns_get_theme_option('header_signin_show') == 1) : ?>
                <li class="d-lg-flex d-none">
                    <?php Egns_Helper::login_register() ?>
                </li>
            <?php endif ?>
            <?php if (class_exists('CSF') && Egns_Helper::egns_get_theme_option('header_sidebar_icon_show') == 1) : ?>
        ...
        <?php if (class_exists('CSF') && Egns_Helper::egns_get_theme_option('header_button_enable') == 1) : ?>
            <a href="<?php echo esc_url(Egns_Helper::egns_get_theme_option('header_button_link', 'url')) ?>" class="primary-btn3 d-xl-flex d-none"><?php echo esc_html(Egns_Helper::egns_get_theme_option('header_button_text')) ?></a>
        <?php endif ?>
